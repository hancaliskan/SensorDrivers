...
static uint8_t tml_Tx(uint8_t *pBuff, uint16_t buffLen) {
    if (HAL_I2C_Master_Transmit(&hi2c2, BOARD_NXPNCI_I2C_ADDR, pBuff, buffLen, 1000) != SUCCESS)
    {
    	Sleep(10);
    	if(HAL_I2C_Master_Transmit(&hi2c2, BOARD_NXPNCI_I2C_ADDR, pBuff, buffLen, 1000) != SUCCESS)
    	{
...
static uint8_t tml_Rx(uint8_t *pBuff, uint16_t buffLen, uint16_t *pBytesRead) {
	...
	ret = HAL_I2C_Master_Receive(&hi2c2, BOARD_NXPNCI_I2C_ADDR, pBuff, 3, 1000);

	if (ret == SUCCESS) {
		if (pBuff[2] != 0) {
			ret = HAL_I2C_Master_Receive(&hi2c2, BOARD_NXPNCI_I2C_ADDR, &pBuff[3], pBuff[2], 1000);
			if (ret == SUCCESS) {
